...
| ~M-x RET cheat RET <command>~ | Open a cheatsheet having given <command>. Needs ivy to work |
| ~M-x RET cheat/<command>~     | All cheatsheets have dedicated interactive functions, for instance ~cheat/adoc~ opens AsciiDoc cheatsheet. |
